// register.js - Complete optimized version with enhanced UX features
document.addEventListener('DOMContentLoaded', () => {
    ...
    customerForm.addEventListener('submit', async (e) => {
        ...
        await handleRegistration('/api/register/pelanggan', formData, 'pelanggan');
    });
    ...
    supplierForm.addEventListener('submit', async (e) => {
        ...
        await handleRegistration('/api/register/supplier', formData, 'supplier');
    });
    ...
    function validateCustomerForm(data) {
        ...
        } else if (!/^08\d{8,13}$/.test(data.no_telp)) {
    ...
    function validateSupplierForm(data) {
        ...
        if (!data.nama) {
            errors.push('Nama rental/perusahaan tidak boleh kosong');
        }
    ...
    async function handleRegistration(apiUrl, formData, userType) {
        try {
            // Validate form data
            const errors = userType === 'pelanggan'
            ...
            showSuccessMessage(result.message || 'Registrasi berhasil!');
    ...
    function setLoadingState(loading) {
        ...
        if (loading) {
            ...
            if (currentRole === 'pelanggan') {
                submitBtn.textContent = 'Daftar sebagai Pelanggan';
            } else {
                submitBtn.textContent = 'Daftar sebagai Supplier';
            }
        }
    }
    ...
    function updateProgress() {
        ...
        if (progress === 100) {
            ...
            progressFill.style.background = 'linear-gradient(90deg, #ffc107, #28a745)';
        } else {
            progressFill.style.background = 'linear-gradient(90deg, #007bff, #6f42c1)';
        }
    }
    ...
    function showFieldError(input) {
        ...
        if (errorMessage) {
            showFieldTooltip(input, errorMessage);
        }
    }
